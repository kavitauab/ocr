...
function classifyDocument($filePath, $fileType) {
        ...
        'input_schema' => [
            ...
            'properties' => [
                'category' => [
                    ...
                    'description' => 'Document category. CRITICAL: classification is primarily driven by the document TITLE/HEADER. Lithuanian/Latvian/Estonian title keywords strongly override visual appearance. Any document titled "PVM sąskaita faktūra", "Sąskaita faktūra", "Sąskaita-faktūra", "VAT Invoice", "Invoice", "Faktūra", "Rēķins" MUST be classified as "invoice" — even if the document is handwritten, partially illegible, a photo/scan, or contains a table that looks like an act. NEVER reject based on the vendor\'s industry or branding (couriers, logistics, telecom, utilities, etc. issue real invoices — their logos and parcel/weight/shipment columns are normal invoice content, not a non-invoice signal). "proforma" only for genuine proforma/advance/prepayment/deposit invoices (title usually includes "Išankstinė" or "Proforma"). "credit_note" for credit notes (title contains "Kreditinė"). "order_confirmation" for order confirmations/purchase orders. "act" ONLY when the title is "Aktas" / "Darbų priėmimo-perdavimo aktas" / "Work acceptance act" and NO invoice title is present. "report" for reports, "contract" for contracts/agreements, "other" for anything else.',
                ],
                'detail' => [
                    'type' => 'string',
    ...
    $requestBody = [
        'model' => $model,
        'max_tokens' => 256,
        'system' => [['type' => 'text', 'text' => 'You are a document classifier. Identify the type of business document shown. FIRST, look at the document title/header (usually top of the first page). If the title contains "PVM sąskaita faktūra", "Sąskaita faktūra", "Sąskaita-faktūra", "VAT Invoice", "Invoice", "Faktūra", or "Rēķins", classify as "invoice" — this overrides any visual judgment about the document looking like a ledger, act, or handwritten note. CRITICAL: NEVER reject a document as non-invoice based on the issuer\'s industry, brand, or logo. Couriers (DPD, DHL, UPS, FedEx, Venipak, LP Express, Omniva), telecom operators, utilities, banks, retailers, hotels, airlines, hosting providers, software vendors, and ANY other type of company can and DO issue real invoices. The presence of a company logo or industry indicator (parcels, packages, weight, kg, shipments, "Siuntos", "Pakuotės", "Paslaugos") is NOT a reason to skip — those are normal line items on invoices for services rendered. The only thing that determines classification is the document TITLE and whether it contains an invoice number, issuer details, and a payable amount. Only use proforma for explicit proforma/advance/prepayment/deposit invoices (title says "Išankstinė" / "Proforma"). A paid invoice or payment receipt that represents the actual invoice is "invoice", not proforma. Statements, extracts, consumption breakdowns, spending breakdowns, reports, account statements, utility usage summaries, and documents labeled "išklotinė", "extract", or "statement" are NOT invoices unless they also clearly contain actual invoice issuance details (supplier invoice number and seller identity). Only classify as "act" when the title explicitly says "Aktas" or "Darbų priėmimo-perdavimo aktas" AND no invoice title is present.', 'cache_control' => ['type' => 'ephemeral']]],
        'tools' => [$tool],
