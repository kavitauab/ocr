...
- [x] Verify with frontend build and production deploy.

## Verification Checklist
- [x] `npm run build` passes.
- [x] Invoice detail `Back` returns to the filtered invoice list.
- [x] Invoice detail `Previous` and `Next` move across the current invoice page/query.
...
- Verification run:
  - `npm run build`
  - `./deploy.sh "Remove sidebar collapse and add invoice detail navigation"`
...
- [x] Verify with PHP syntax checks and production deploy.
...
- [x] Production deploy completes successfully.
