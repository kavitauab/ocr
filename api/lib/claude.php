...
function classifyDocument($filePath, $fileType) {
    ...
    $requestBody = [
        'model' => $model,
        'max_tokens' => 256,
        'system' => [['type' => 'text', 'text' => 'You are a document classifier. Identify the type of business document shown. FIRST, look at the document title/header (usually top of the first page). If the title contains "PVM sąskaita faktūra", "Sąskaita faktūra", "Sąskaita-faktūra", "VAT Invoice", "Invoice", "Faktūra", or "Rēķins", classify as "invoice" — this overrides any visual judgment about the document looking like a ledger, act, or handwritten note. Only use proforma for explicit proforma/advance/prepayment/deposit invoices (title says "Išankstinė" / "Proforma"). A paid invoice or payment receipt that represents the actual invoice is "invoice", not proforma. Statements, extracts, consumption breakdowns, spending breakdowns, reports, account statements, utility usage summaries, and documents labeled "išklotinė", "extract", or "statement" are NOT invoices unless they also clearly contain actual invoice issuance details (supplier invoice number and seller identity). Only classify as "act" when the title explicitly says "Aktas" or "Darbų priėmimo-perdavimo aktas" AND no invoice title is present.', 'cache_control' => ['type' => 'ephemeral']]],
        'tools' => [$tool],
