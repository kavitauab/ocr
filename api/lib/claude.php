...
function getAllExtractionFields() {
    return [
        ...
        'invoiceNumber' => ['type' => 'string', 'description' => 'The invoice number/ID. If the document shows a series and number separately (e.g. "Serija M Nr. 786646" or "Serija TE2023 Nr. 285"), concatenate them directly WITHOUT any dash or separator (e.g. "M786646", "TE2023285"). Remove prefixes like "Nr.", "No.", "Serija" etc. If the invoice number is already a single value (e.g. "SJ-152138"), keep it exactly as-is without modifying existing separators.'],
        ...
        'vendorName' => ['type' => 'string', 'description' => 'Legal name of the company/person ISSUING the invoice (the seller, the party requesting payment). Identify the seller using these signals, in order of importance: (1) the company whose bank account / IBAN appears in the "Mokėjimo nurodymas pervesti įmoką" / "Bank details" / "Pay to" block; (2) the company whose details appear adjacent to the "PVM SĄSKAITA FAKTŪRA" / "Sąskaita faktūra" / "Invoice" header and the invoice number; (3) the company whose details appear next to "Sąskaitos data" / "Issue date" and "Apmokėti iki" / "Due date"; (4) the entity in the footer with full registration details, ISO certificates, or "Duomenys apie įmonę kaupiami ir saugomi juridinių asmenų registre" disclaimer. The BUYER is the recipient (the party owing payment) — typically just an address block with no bank details and no invoice metadata. Brand logo position is NOT a reliable signal — the seller\'s logo can be on either side. When two parties are present, the SELLER is the one with payment instructions, the BUYER is the other one. Do not use venue, hotel, store, branch, or brand/trading name when a different legal seller is shown.'],
        ...
        'vendorCompanyCode' => ['type' => ['string', 'null'], 'description' => 'Vendor company registration code (Lithuanian "Įmonės kodas" / "Kodas", Latvian "Reģ. Nr.", numeric business identifier — NOT the VAT ID). Return digits only, no prefixes.'],
        'buyerName' => ['type' => ['string', 'null'], 'description' => 'Legal name of the buyer/recipient (the party owing payment). The buyer is the company whose details appear AS A RECIPIENT BLOCK — typically with no bank account number, no "Sąskaitos data" / "Invoice date" label, no IBAN / payment instructions. The buyer is NOT the company shown in the "Mokėjimo nurodymas" / "Pay to" / bank details block — that is the seller. If unsure, the buyer is whichever party is NOT receiving the payment.'],
        ...
        'buyerCompanyCode' => ['type' => ['string', 'null'], 'description' => 'Buyer company registration code (Lithuanian "Įmonės kodas" / "Kodas", Latvian "Reģ. Nr.", numeric business identifier — NOT the VAT ID). Return digits only, no prefixes.'],
        ...
        'totalAmount' => ['type' => 'number', 'description' => 'The INVOICE\'S OWN total including tax — the value of the goods/services billed on THIS document. Use the line labeled "Sąskaitos suma su PVM" / "Suma su PVM" / "Iš viso su PVM" / "Total incl. VAT" / "Grand Total" / "Amount incl. tax". CRITICAL: this is NOT the net amount left to pay after applying a prior account balance, overpayment, credit, or carried-forward debt. Lithuanian invoices often show a running balance line ("Skola(+)/Permoka(-)", "Ankstesnė skola", "Permoka") and then a final "Iš viso mokėtina suma" / "Mokėtina suma" / "Apmokėti" line that nets the invoice against that balance — IGNORE that net-payable line for this field. If the invoice sum is 65.90 but "mokėtina suma" shows 0.00 because of an overpayment, totalAmount is 65.90. When the invoice has multiple VAT categories (standard, reverse-charge "atvirkštinis PVM", non-taxable "neapmokestinama"), totalAmount is the document\'s stated invoice sum with VAT, which already aggregates them. Never use Sub Total/Subtotal here when an invoice sum with VAT exists.'],
...
function reviewInvoiceMonetarySummary($filePath, $fileType, $model) {
    ...
    $contentBlocks[] = [
        'type' => 'text',
        'text' => 'Re-check only the monetary summary. For totalAmount return the invoice\'s OWN sum WITH tax ("Sąskaitos suma su PVM" / "Total incl. VAT"), NOT the net amount left to pay after a prior balance / overpayment / credit ("Iš viso mokėtina suma" / "Mokėtina suma" can be 0 or negative due to a "Skola/Permoka" line — ignore that for totalAmount). Also return subtotal, tax, and any extra freight/shipping/insurance charges.',
    ];
...
10. Distinguish Sub Total/Subtotal from the invoice total. totalAmount must be the invoice's OWN sum WITH tax (the 'Sąskaitos suma su PVM' / 'Total incl. VAT' / 'Grand Total' line), not the subtotal. Do NOT use the net amount left to pay after a prior account balance, overpayment, or credit — Lithuanian invoices show a 'Skola(+)/Permoka(-)' balance line then an 'Iš viso mokėtina suma' / 'Mokėtina suma' net line which can be 0 or negative; that net line is NOT totalAmount. If 'Sąskaitos suma su PVM' = 65.90 but 'mokėtina suma' = 0.00 due to an overpayment, totalAmount = 65.90.
...
function classifyDocument($filePath, $fileType) {
        ...
        'input_schema' => [
            ...
            'properties' => [
                'category' => [
                    ...
                    'description' => 'Document category. CRITICAL: classification is primarily driven by the document TITLE/HEADER. Lithuanian/Latvian/Estonian title keywords strongly override visual appearance AND financial layout. Any document titled "PVM sąskaita faktūra", "Sąskaita faktūra", "Sąskaita-faktūra", "VAT Invoice", "Invoice", "Faktūra", "Rēķins" MUST be classified as "invoice". NEVER reject based on the vendor\'s industry or branding (couriers, logistics, telecom, utilities, etc. issue real invoices — their logos and parcel/weight/shipment columns are normal invoice content). "proforma" only for genuine proforma/advance/prepayment/deposit invoices (title usually includes "Išankstinė" or "Proforma"). "credit_note" for credit notes (title contains "Kreditinė"). "order_confirmation" for order confirmations/purchase orders. "act" — ANY document whose title contains "Aktas", "AKTAS", "Atliktų darbų priėmimo", "Darbų priėmimo-perdavimo aktas", "Priėmimo-perdavimo aktas", "Work acceptance act", or "Acceptance certificate" MUST be classified as "act" — EVEN IF the document also contains financial breakdowns with amounts and VAT (Lithuanian acts often summarize the work performed and its cost, but they are NOT invoices). The presence of subtotal / VAT / total / customer + supplier details does NOT make an act into an invoice. Only override act → invoice when a separate explicit invoice title (PVM sąskaita faktūra / Sąskaita faktūra / Faktūra / VAT Invoice / Invoice / Rēķins) is ALSO visible on the same page. "report" for reports, "contract" for contracts/agreements, "other" for anything else.',
                ],
                'detail' => [
                    'type' => 'string',
    ...
    $requestBody = [
        'model' => $model,
        'max_tokens' => 256,
        'system' => [['type' => 'text', 'text' => 'You are a document classifier. Identify the type of business document shown. FIRST, look at the document title/header (usually top of the first page). The title is the SINGLE most important signal — it overrides visual layout, financial breakdowns, and content style. If the title contains "PVM sąskaita faktūra", "Sąskaita faktūra", "Sąskaita-faktūra", "VAT Invoice", "Invoice", "Faktūra", or "Rēķins", classify as "invoice". CRITICAL: NEVER reject a document as non-invoice based on the issuer\'s industry, brand, or logo. Couriers (DPD, DHL, UPS, FedEx, Venipak, LP Express, Omniva), telecom operators, utilities, banks, retailers, hotels, airlines, hosting providers, software vendors, and ANY other type of company can and DO issue real invoices. The presence of a company logo or industry indicator (parcels, packages, weight, kg, shipments, "Siuntos", "Pakuotės", "Paslaugos") is NOT a reason to skip — those are normal line items. EQUALLY CRITICAL: a financial breakdown table (subtotal, VAT, total) on a document does NOT automatically make it an invoice. Lithuanian work acceptance acts ("Atliktų darbų priėmimo - perdavimo AKTAS", "Aktas", "Priėmimo-perdavimo aktas") routinely include the cost of the work performed with VAT — they are still ACTS, not invoices. If you see "AKTAS" / "Aktas" in the title and there is NO separate invoice title (PVM sąskaita faktūra / Sąskaita faktūra / Faktūra / VAT Invoice / Invoice / Rēķins) ALSO visible on the same page, you MUST classify as "act" regardless of any monetary table below. Only use proforma for explicit proforma/advance/prepayment/deposit invoices (title says "Išankstinė" / "Proforma"). A paid invoice or payment receipt that represents the actual invoice is "invoice", not proforma. Statements, extracts, consumption breakdowns, spending breakdowns, reports, account statements, utility usage summaries, and documents labeled "išklotinė", "extract", or "statement" are NOT invoices unless they also clearly contain actual invoice issuance details (supplier invoice number and seller identity).', 'cache_control' => ['type' => 'ephemeral']]],
        'tools' => [$tool],
